...
def _command_gate(name: str, commands: list[list[str]], command_runner: Callable[[Sequence[str]], CommandResult]) -> GateResult:
    failures: list[str] = []
...
def _single_case_pass(output: str, case_number: int) -> bool:
    ...
    expected = (
        rf"process_ok:\s*1/1",
        rf"environment_errors:\s*0/1",
        h1_pattern,
        rf"hygiene_pass:\s*1/1",
        rf"incomplete_url_pass:\s*1/1",
        rf"contract_oracle_pass:\s*1/1",
    )
    ...
    *,
    ...
    for case_number in CRITICAL_CASES:
        for attempt in range(1, REPEAT_CASES.get(case_number, 1) + 1):
            output_dir = ROOT / "regression-results" / "critical-stability" / run_id / f"E{case_number:02d}-attempt{attempt}"
            result = command_runner(_runner_command(
                codex=codex,
    ...
    return GateResult("B: critical stability", not failures, tuple(failures))
...
def full_regression_gate(
    *,
    ...
    result = command_runner(_runner_command(codex=codex, installed_skill_root=installed_skill_root, model=model))
    required = (
        r"process_ok:\s*42/42",
        r"environment_errors:\s*0/42",
        r"h1_pass:\s*40/42",
        r"hygiene_pass:\s*42/42",
        r"incomplete_url_pass:\s*42/42",
        r"contract_oracle_pass:\s*42/42",
    )
    passed = result.returncode == 0 and all(re.search(pattern, result.output) for pattern in required)
    return GateResult("C: full E1-E42", passed, () if passed else ("full regression acceptance did not pass",))
...
def orchestrate(
    *,
    ...
    gate_b = critical_fn() if critical_fn else critical_stability_gate(codex=codex, installed_skill_root=installed_skill_root, model=model)
    results.append(gate_b)
    if not gate_b.passed or mode == "critical":
        return results

    gate_c = full_fn() if full_fn else full_regression_gate(codex=codex, installed_skill_root=installed_skill_root, model=model)
...
def main() -> int:
    ...
    results = orchestrate(mode=mode, installed_skill_root=args.installed_skill_root, codex=args.codex, model=args.model)
