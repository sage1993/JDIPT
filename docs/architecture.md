...
`logic-validation.md`는 법적 결론을 새로 만드는 도구가 아니라 이미 구성한 논증을 감사하는 Gate다.
...
평가계약의 개수와 실제 LLM 호출 수를 분리한다. 과거 fixture를 삭제하지 않으면서 대표 시나리오 하나에 여러 machine oracle을 연결해 중복 실행을 줄인다.

단일 실행원본: `skills/law-interpretation-request/evals/suite-manifest.json`
...
- **Core 14**: PR의 치명적 fail-closed 계약과 Golden Case 안정성 검증
- **Full active 26**: 릴리스 전 실제 LLM 회귀
- **Legacy 20**: 과거 회귀 재현·진단용. 기본 실행에서는 제외하지만 fixture와 oracle은 보존
- **Catalog E1~E46**: 추적 가능한 전체 평가 자산

신규 v0.2.3 behavior는 E43~E46 네 케이스다.
...
실행은 `scripts/run_eval_suite.py`가 담당하고 release orchestration은 Gate B에서 Core, Gate C에서 Full active를 실행한다. 상세 mapping은 `docs/evaluation-suites.md`를 따른다.
