...
- Runtime activation is explicit and turn-scoped: a successful bundled `register_material_proposition` call writes the structured proposition ledger under `PLUGIN_DATA` for the exact `session_id` and `turn_id`, sets `registry_active=true`, and is the only activation signal for the Stop gate. A missing record on an unrelated turn is a no-op.
- The registry input must not contain a model-authored render slots; the runtime deterministically generates those slots from the structured fields. Runtime state contains compact metadata only and must not contain secrets, a transcript, or a full source document.
...
- The source/evidence anchors and `render_contract` slots preserve the source-specific operative action. When present, use `render_contract` slot text → `source_proposition` → `evidence_span` in that order before attempting a free paraphrase.
