...
- Registry lifecycle has one authoritative writer: `RegistryService` owns exact-turn PENDING activation, proposition registration, bounded enforcement, and registry dispositions. Persistence, activation, MCP transport, and Stop orchestration do not write registry lifecycle fields directly.
- Registry transitions are atomic and fail closed. A bounded exact-turn lock plus complete-state validation prevents duplicate writers, split-brain state, partial PENDING → ACTIVE transitions, and stale cross-turn state reuse.
