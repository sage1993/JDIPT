...
- Runtime activation (MUST): every `UserPromptSubmit` creates one authoritative Turn Anchor under the trusted runtime root, increments its global `epoch`, and invalidates any non-closed transaction from a prior epoch. An explicit `$law-interpretation-request` prompt uses that boundary; it does not supply authority fields to MCP.
- Runtime capability isolation (MUST): `begin_runtime_turn` atomically claims the current anchor and issues a server-generated opaque `turn_capability`. The server stores only its SHA-256 digest and binds every `submit_material_obligation_ledger`, `register_material_proposition`, and `finalize_runtime_turn` call to the current epoch, anchor, and canonical transaction. The model supplies no `session_id`, `turn_id`, `plugin_data`, `_runtime_session_id`, `_runtime_turn_id`, or `_runtime_plugin_data`. The registry accepts structured proposition fields only; the model must not supply `mandatory_render_clause`, which is generated deterministically from the registered fields.
- Runtime state isolation (MUST): keep the compact ledger digest, proposition metadata, transaction state, and bounded reconciliation evidence only in the canonical runtime registry under the trusted runtime root; never store plaintext capabilities, a transcript, or a full source document. V1 permits one globally active JDIPT transaction. Stop passes only when the current epoch transaction is `CLOSED`; a plain unrelated turn with no active transaction remains a no-op.
- Immutable source-ledger order (MUST): after `begin_runtime_turn`, finish the bounded official-source retrieval and verify each available source before the first `submit_material_obligation_ledger` call. Submit one complete ledger with its verified evidence set before registering propositions. Never submit an empty or provisional evidence array while retrieval is still in progress; the ledger digest is immutable and later evidence additions are rejected. Use `SOURCE_UNRESOLVED` only after the targeted retrieval is complete and no verifiable source was found.
- Exact evidence binding (MUST): include a separate `verified_source_evidence` reference for each distinct verified clause/span; distinct references may share one `source_id`. Each `CLOSED` proposition's `source_id`, authority, title, locator, evidence span, and temporal fields must exactly match one ledger reference. A matching `source_id` alone is insufficient. Put every proposition-specific citation in the single immutable ledger before registering propositions, and make the proposition's evidence span support its complete relation. Set `required_authority` explicitly to the authority warranted by that verified source (`PRIMARY` for statute/regulation/ordinance; `GUIDANCE` for a proposition directly governed by official guidance); do not rely on the default or weaken the requirement merely to pass closure.
- Runtime identity boundary (MUST): the authoritative boundary is the atomic `current-turn-anchor.json` record and the canonical transaction registry. The plugin does not register a PreToolUse runtime hook; legacy injector output is never an authority source.
...
- Material Proposition Schema: carry the canonical MCP fields `proposition_id`, `status`, `materiality`, `subject`, `condition`, `procedure`, `modality`, `legal_action`, `operative_verb_lexeme`, `legal_object`, `legal_effect`, `polarity`, `relation_type`, `base_proposition_id`, `exception_proposition_id`, `base_rule`, `exception_rule`, `required_authority`, `required_temporal_status`, `required_source_type`, `source_id`, `authority_kind`, `source_title`, `source_locator`, `evidence_span`, `temporal_status`, and `temporal_render_text`. `mandatory_render_clause` is output-only and generated by the registry; it is never an input field.
- Canonical proposition contract (MUST): `status` is the closure state, `legal_effect` is the resulting legal status/effect, `relation_type` is the base/exception relation, and `source_id`/`source_locator`/`evidence_span` carry verified provenance. The legacy names `closure_status`, `direct_source`, `relation_to_base_or_exception`, and `resulting_status_or_effect` are obsolete/invalid MCP input fields; do not translate them or add them to the MCP schema.
- Linked base/exception relation (MUST): when a material main rule and exception are identified, preserve their stable proposition IDs when available and register the relation type, base rule, exception rule, condition, procedure, legal actor, legal action, legal object, and legal effect as one source-linked structure. Do not rely on document-wide coexistence of the two values.
...
- Exact-effect default path: when a CLOSED material proposition has a specific legal action/effect, use the generated `mandatory_render_clause`, then the verified `evidence_span`, and copy that operative relation into the final answer before writing any free paraphrase. Never replace the operative clause with a weaker generic verb.
...
- Repair hard stop: on any material mismatch, perform one targeted repair from the generated `mandatory_render_clause` → verified `evidence_span` → source-equivalent close paraphrase, then one bounded re-check. Do not use a new free-form paraphrase as the first repair.
...
4. 각 명시적 JDIPT 실행에서는 먼저 `begin_runtime_turn`을 호출한다. 반환된 opaque capability를 보관한 채 필요한 공식 자료 검색과 원문 확인을 끝내고, 검증된 출처와 obligation/proposition 연결을 포함한 ledger를 한 번만 `submit_material_obligation_ledger`에 제출한다. 검색이 끝나기 전 빈 원장이나 임시 `SOURCE_UNRESOLVED` 원장을 제출하지 않는다. 서로 다른 절·문장 span은 `source_id`가 같더라도 각각 `verified_source_evidence` 항목으로 넣고, 각 `CLOSED` proposition의 EvidenceRef 필드는 그중 한 항목과 정확히 일치시킨다. `required_authority`도 확인된 source kind에 맞게 명시한다. 그다음 source-supported proposition을 식별할 때마다 bundled `register_material_proposition`에 완전한 구조화 필드와 근거에 맞는 `OPEN`/`CLOSED` 상태를 등록하고, 모든 proposition 등록 후 반드시 `finalize_runtime_turn`을 호출한다. 출처가 확인된 일반 법규칙은 그 법적 관계가 source로 완결된 범위에서 `CLOSED`로 등록할 수 있으며, 미확인 사업지 사실은 별도의 `OPEN` proposition으로 둔다. `CLOSED` proposition의 condition·procedure·subject·legal action·object·effect는 등록한 evidence span이 실제로 뒷받침해야 한다. `mandatory_render_clause`와 runtime identity fields는 입력하지 않는다. 본칙과 예외가 연결되면 `relation_type`, 가능한 경우의 base/exception proposition ID, base/exception rule, 조건·절차·법적 행위·대상·효과를 함께 보존한다. capability가 현재 epoch와 맞지 않거나 transaction이 이미 닫혔으면 재사용하지 말고 fail-closed 상태로 처리한다.
...
**Typed runtime field contract:** For ledger evidence and proposition evidence, `authority_kind` must be one of `statute`, `regulation`, `ordinance`, `precedent`, `interpretation`, `guidance`, or `other`; evidence `temporal_status` must be `CURRENT_CONFIRMED`, `HISTORICAL_CONFIRMED`, or `CURRENT_UNRESOLVED`. Proposition `required_temporal_status` is a different field and must be `CURRENT`, `HISTORICAL`, or `ANY`. `modality` must be `MAY`, `MUST`, `MUST_NOT`, or `MAY_NOT`; put qualifications in `condition`, `procedure`, or `legal_effect`, not in `modality`. `required_authority` must be `PRIMARY`, `PRECEDENT`, `INTERPRETATION`, `GUIDANCE`, or `ANY`. Within each obligation, list each source ID once in `evidence_source_ids`, even if `verified_source_evidence` contains multiple distinct spans for that source.
