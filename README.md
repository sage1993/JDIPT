...
현재 공개 버전은 **v0.2.3**입니다. v0.2 계열은 `Legal Issue Mapping → Legal Interpretation → Source Completeness / Counterevidence → Logic Validation → Answer Rendering` 파이프라인, 결론 우선 4단 출력, explicit-only 호출, 참조 별표·별지서식 resolution hard gate, 최종 rendering/source URL hard gate와 결정론적 회귀검증을 제공합니다.

> 현재 배포 상태: **JDIPT v0.2.3 RELEASE GATE = PASS**
...
자동 Skill 선택은 v0.2.3 release gate의 요구사항이 아닙니다.
...
2026-08-30 기준 release validation 결과:
...
| Gate C process | **26/26** |
| Gate C environment errors | **0/26** |
| Gate C hygiene | **26/26** |
| Gate C URL | **26/26** |
| Gate C oracle | **26/26** |
...
기존 v0.2.2 검증 상세는 [`docs/validation/v0.2.2-source-rendering.md`](docs/validation/v0.2.2-source-rendering.md)에 보존합니다.
...
- **v0.2.3:** Authority / Temporal / Claim-level Evidence 계약 강화, Core14/Full26 평가 체계, E02/E03/E18/E37/E44 안정화
